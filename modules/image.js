...
import { CorvanError } from '../utils/errors.js';

const POLL_INTERVAL = 2000;
const POLL_TIMEOUT  = 120_000;

export class ImageModule {
  /** @param {import('../client.js').CorvanClient} client */
  constructor(client) {
    this._client = client;
  }

  /**
   * Generate an image from a text prompt using corvan.v1.
  ...
  async _poll(jobId, onProgress) {
    ...
    while (Date.now() < deadline) {
      ...
      if (status.status === 'failed') {
        throw new CorvanError(status.error ?? 'Image generation failed');
      }
    }

    throw new CorvanError('Image generation timed out');
  }
