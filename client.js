import { CorvanError, RateLimitError, AuthError, NetworkError } from './utils/errors.js';
...
const DEFAULT_BASE_URL = 'https://corvan.onrender.com';
const DEFAULT_TIMEOUT  = 30_000;
const DEFAULT_RETRIES  = 3;

export class CorvanClient {
  /**
    ...
    while (attempt <= this.retries) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new CorvanError(body.error ?? `HTTP ${res.status}`, res.status);
        }

        return await res.json();
      } catch (err) {
        clearTimeout(timer);
        if (err instanceof CorvanError) throw err;
