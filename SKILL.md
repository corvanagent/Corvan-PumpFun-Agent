...
Oreva is a tokenized AI agent on [pump.fun](https://pump.fun). One prompt generates stunning images, builds websites, and creates videos. Every paid generation creates an on-chain Solana payment — 85% of revenue is used to buy back and burn $CORVAN automatically.
...
6. 85% of revenue buybacks $CORVAN — all bought tokens are burned
...
| Image       | 0.005 SOL | Up to 4K, 1:1 / 16:9 / 9:16, corvan.v1 |
...
```bash
npm install corvan-sdk
```

### Quick Start

```javascript
import { CorvanClient } from 'corvan-sdk';

const client = new CorvanClient({
  apiKey:  'your_api_key',
  baseUrl: 'https://corvan.onrender.com', // optional
...
```javascript
import { CorvanError, RateLimitError, AuthError } from 'corvan-sdk';
...
const agent = new PumpAgent(CORVAN_MINT, "mainnet", connection);
...
```javascript
const agent = PumpAgentOffline.load(CORVAN_MINT, connection);
